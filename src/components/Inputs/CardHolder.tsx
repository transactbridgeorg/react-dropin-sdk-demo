...
import { setCardHolderName } from "../../redux/cardSlice";
...
const CardHolder = () => {
  ...
  useEffect(() => {
    ...
    const instance = window?.TBDropin?.create('tb_cardHolder', CARD_HOLDER_CONFIG);
    instance?.mount('#cardHolder');
    instance?.on("change", (data: { value?: string; isValid?: boolean; }) => {
        const {isValid = false} = data as { isValid?: boolean; };
        dispatch(setCardHolderName(isValid));
    });
