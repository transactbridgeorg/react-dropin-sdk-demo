...
import { setCardExpiry } from "../../redux/cardSlice";
...
const CardExpiry = () => {
  ...
  useEffect(() => {
    ...
    const instance = window?.TBDropin?.create('tb_cardExp', CARD_EXPIRY_CONFIG);
    instance?.mount('#cardExp');
    instance?.on("change", (data: { value?: string; isValid?: boolean; }) => {
        const {isValid = false} = data as { isValid?: boolean; };
        dispatch(setCardExpiry(isValid));
    });
